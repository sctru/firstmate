...
console.log(JSON.stringify({
  liveReplacementArmPids: liveArms().map((arm) => arm.pid),
  carriedSuccessorOutcomeDeliveries: replacement.prompts.filter((message) => message.includes("signal: replacement-successor actionable outcome")).length,
  settledOldOutcomeReplays: replacement.prompts.filter((message) => message.includes("signal: replacement-race actionable outcome")).length,
  cleanupFailureAlerts: replacement.prompts.filter((message) => message.includes("could not clear a delivered replacement-session actionable wake")).length,
  automaticArmRetained: redundant.details?.ok === true && String(redundant.details.message).includes("unchanged"),
}));
writeFileSync(process.env.FM_STOP_FILE, "stop\n");
process.exit(0);
EOF
