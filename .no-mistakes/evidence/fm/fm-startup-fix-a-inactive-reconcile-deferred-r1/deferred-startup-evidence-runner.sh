...
. "/Users/kunchen/.no-mistakes/worktrees/52b07e9083e7/01M1FK78DKDKWRQXY46R50PT66/tests/lib.sh"
...
rec=$(new_world reviewer-evidence)
IFS='|' read -r home root log <<EOF
$rec
EOF
printf '%s\n' $$ > "$home/state/.lock"
printf '../other-home\n' > "$home/.fm-secondmate-home"
printf '$ fm-startup-network.sh start --locked 1 --harvest-pid %s\n' "$$"
FM_FAKE_BOOTSTRAP_LOG="$log" run_stage "$home" "$root" start --locked 1 --harvest-pid $$
printf 'start returned; waiting explicitly for detached publication...\n'
FM_FAKE_BOOTSTRAP_LOG="$log" run_stage "$home" "$root" wait 30 >/dev/null
printf '\n$ fm-startup-network.sh report\n'
run_stage "$home" "$root" report
printf '\n$ persisted wake queue\n'
cat "$home/state/.wake-queue"
printf '\n$ fm-wake-drain.sh (captain-facing output)\n'
FM_HOME="$home" FM_STATE_OVERRIDE="$home/state" "$DRAIN" 2>"$home/drain.err"
printf '\n$ durable acknowledgement instruction\n'
cat "$home/drain.err"
