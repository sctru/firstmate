...
. "/Users/kunchen/.no-mistakes/worktrees/52b07e9083e7/01M1MNQNACK2TGCPP57VAHN6H2/tests/lib.sh"
...
SH
  ...
  printf '%s\n' "$parallel" > "$EVIDENCE_DIR/fm-bearings-large-local-projection.json"
  serial_sha=$(printf '%s' "$serial" | shasum -a 256 | awk '{print $1}')
  parallel_sha=$(printf '%s' "$parallel" | shasum -a 256 | awk '{print $1}')
  jq -n --argjson serial_seconds "$serial_elapsed" \
    --argjson concurrent_seconds "$parallel_elapsed" --argjson seconds_saved "$saved" \
    --arg serial_sha256 "$serial_sha" --arg concurrent_sha256 "$parallel_sha" \
    --argjson worker_rows "$(printf '%s' "$parallel" | jq '.in_flight | length')" \
    --argjson done_backlog_rows 300 \
    '{scenario:"real fm-bearings-snapshot CLI against synthetic large local home", serial_seconds:$serial_seconds, concurrent_seconds:$concurrent_seconds, seconds_saved:$seconds_saved, serial_projection_sha256:$serial_sha256, concurrent_projection_sha256:$concurrent_sha256, projections_byte_identical:($serial_sha256 == $concurrent_sha256), worker_rows:$worker_rows, expected_worker_rows:5, done_backlog_rows:$done_backlog_rows}'
