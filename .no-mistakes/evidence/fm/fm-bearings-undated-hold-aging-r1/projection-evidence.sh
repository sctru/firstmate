...
. "/Users/kunchen/.no-mistakes/worktrees/52b07e9083e7/01M1GENG7DW7JZB4G175VSW66H/tests/lib.sh"
...
test_remote_ledgers_share_one_concurrent_budget_and_fall_back_to_cache
test_a_remote_home_without_any_ledger_is_explicitly_unreadable_without_remote_compute
home=$(make_home evidence-undated-aging)
mkdir -p "$home/data"
...
BACKLOG
fakebin=$(make_fakebin "$home")
run "$home" "$fakebin" --json | jq . > "$EVIDENCE/default-projection.json"
run "$home" "$fakebin" --json --all-decisions | jq . > "$EVIDENCE/all-decisions-projection.json"
