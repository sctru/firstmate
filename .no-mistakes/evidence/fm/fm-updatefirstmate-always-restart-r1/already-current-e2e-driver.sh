...
evidence_case=$(new_case reviewer-e2e)
add_repo_backed_mate "$evidence_case" SSHHIP
arm_answer "$evidence_case" SSHHIP
before=$(git -C "$evidence_case/SSHHIP-home" rev-parse HEAD)

echo '=== 1. /updatefirstmate mechanical output ==='
update_out=$(run_update_in_case "$evidence_case")
printf '%s\n' "$update_out"
restart_line=$(printf '%s\n' "$update_out" | grep '^restart-secondmates:')
ids=${restart_line#restart-secondmates: }

echo
echo '=== 2. restart action consumed from update output ==='
printf 'invocation: fm-secondmate-restart.sh %s\n' "$ids"
# shellcheck disable=SC2086
restart_out=$(run_restart "$evidence_case" $ids)
restart_rc=$?
printf '%s\n' "$restart_out"
printf 'exit-status: %s\n' "$restart_rc"

echo
echo '=== 3. end-user-visible terminal chronology (persist before replace) ==='
grep -n -E '^Firstmate instruction waiting: |^/exit$' "$evidence_case/fake/literal"

echo
echo '=== 4. home safety checks ==='
after=$(git -C "$evidence_case/SSHHIP-home" rev-parse HEAD)
printf 'HEAD-before: %s\nHEAD-after:  %s\n' "$before" "$after"
printf 'working-tree-status: %s\n' "$(git -C "$evidence_case/SSHHIP-home" status --porcelain)"
