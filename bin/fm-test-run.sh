...
family_for_basename() {
  case "$1" in
    ...
    fm-bootstrap.test.sh|fm-bootstrap-network-parallel.test.sh|fm-intake-classify.test.sh|fm-fleet-sync.test.sh|fm-gate-refuse.test.sh|fm-gotmp.test.sh|\
...
tests/fm-intake-classify.test.sh 9056
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-dispatch-resolve.sh)
      printf '%s\n' "__script__:fm-dispatch-resolve.test.sh"
      ;;
    bin/fm-intake-classify.sh)
      printf '%s\n' "__script__:fm-intake-classify.test.sh"
      ;;
    bin/fm-env-lib.sh)
      # The one .env accessor, sourced by bin/fm-x-lib.sh (Relay token) and
      # the typed dispatch and intake classifiers (TYPESAFE_API_KEY).
      printf '%s\n' pr-forge
      printf '%s\n' "__script__:fm-dispatch-resolve.test.sh"
      printf '%s\n' "__script__:fm-intake-classify.test.sh"
      ;;
