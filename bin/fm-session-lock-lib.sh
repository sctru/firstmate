...
# rewake; bin/fm-turnend-guard.sh uses it in --claude mode to tell a live foreign
# owner from uncertainty before advising instead of blocking. Two signals decide
# ownership, either one sufficient: the recorded pid is a member of this
# process's contiguous harness ancestry, or the trusted Claude session id below
# matches the id recorded beside a live lock. Neither signal ever fails open:
# no id, no sidecar, an untrusted id, or a different recorded id leaves the
# ancestry verdict exactly as it was.
