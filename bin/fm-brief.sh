...
DOD=$(fm_dod_block "$MODE" "$ID" "$PAUSED_VERB") || exit 1
...
# Project memory
If \`AGENTS.md\` exists, if only a legacy \`CLAUDE.md\` exists, or if this task produced durable project-intrinsic knowledge, run \`$FM_ROOT/bin/fm-ensure-agents-md.sh .\` in the worktree.
The helper writes only \`AGENTS.md\` and leaves any \`CLAUDE.md\` as found.
